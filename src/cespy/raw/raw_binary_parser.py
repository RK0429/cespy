...
class OptimizedBinaryParser:
    ...
    def __init__(self, file_path: Union[str, Path]):
        ...
        _logger.debug("OptimizedBinaryParser initialized for %s (%d bytes)",
    ...
    def read_block(
        ...
        if offset + num_bytes > self.file_size:
            raise ValueError(f"Read would exceed file size: {offset + num_bytes} > {self.file_size}")
    ...
    def read_interleaved(
        ...
        bytes_per_sample = sum(BINARY_FORMATS[fmt].bytes_per_value for fmt in trace_formats)
        ...
        for trace_idx, fmt in enumerate(trace_formats):
            ...
            for point_idx in range(num_points):
                ...
                value_bytes = raw_bytes[sample_offset:sample_offset + fmt_info.bytes_per_value]
                ...
                if fmt == DataFormat.COMPLEX64:
                    ...
                    value = struct.unpack(f'{byte_order}{fmt_info.struct_format}', value_bytes)[0]
    ...
    def read_fast_access(
        ...
        return self.read_sequential(offset, num_traces, num_points, trace_formats, byte_order)
    
    def detect_format(self, offset: int, sample_size: int = 1000) -> Tuple[DataFormat, str]:
        ...
        # Read sample data
        self._file_handle.seek(offset)
        sample_bytes = self._file_handle.read(sample_size * 16)  # Max size for complex128
        ...
        for fmt in DataFormat:
            for order in ['<', '>']:
                try:
                    ...
                    # Parse values
                    values = np.frombuffer(sample_bytes[:num_values * fmt_info.bytes_per_value], dtype=dtype)
        ...
        _logger.info("Detected format: %s, byte order: %s", best_format.value, best_order)
...
def benchmark_parser(file_path: Path, num_traces: int = 10, num_points: int = 100000) -> Dict[str, float]:
    ...
    for method, time_taken in results.items():
        ...
        _logger.info("%s: %.3f seconds (%.1f MB/s)", method, time_taken, throughput_mbps)
