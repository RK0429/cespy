...
class StreamProcessor(ABC):
    ...
    @abstractmethod
    def process_chunk(self, trace_name: str, step: int, 
                     time_data: NDArray, trace_data: NDArray) -> Optional[Any]:
...
class RawFileStreamer:
    ...
    def __init__(self, raw_file: Union[str, Path], config: Optional[StreamConfig] = None):
        ...
        _logger.info("RawFileStreamer initialized: %d traces, %d steps, %d points",
                    self.num_traces, self.num_steps, self.num_points)
...
class MinMaxProcessor(StreamProcessor):
    ...
    def process_chunk(self, trace_name: str, step: int,
                     time_data: NDArray, trace_data: NDArray) -> None:
...
class ThresholdCrossingProcessor(StreamProcessor):
    ...
    def process_chunk(self, trace_name: str, step: int,
        ...
        if key in self._last_values and len(trace_data) > 0:
            ...
            if self.rising and last_val < self.threshold <= first_val:
                ...
                t_cross = self._interpolate_crossing(
                    self._last_times[key], float(time_data[0]),
                    last_val, first_val
                )
        ...
        for idx in crossing_indices:
            ...
            t_cross = self._interpolate_crossing(
                float(time_data[idx]), float(time_data[idx + 1]),
                float(real_data[idx]), float(real_data[idx + 1])
            )
            self.crossings[trace_name][step].append(t_cross)
    ...
    def _interpolate_crossing(self, t1: float, t2: float, v1: float, v2: float) -> float:
