...
class PlatformManager:
    ...
    def _detect_platform(self) -> None:
        ...
        _logger.info("Platform detected: %s %s (%s, %d cores, %.1f GB RAM)",
                    os_type.value, architecture.value, os_version, 
        ...
        if simulator == Simulators.LTSPICE:
            ...
            if self.info.is_windows:
                ...
                paths.extend([
                    ...
                    Path("/home/linuxbrew/.linuxbrew/bin/ngspice"),  # Linux Homebrew
                ])
    ...
    def get_cache_directory(self) -> Path:
        ...
        """
        if self.info.is_windows:
            cache_dir = Path(os.environ.get('LOCALAPPDATA', '~')).expanduser() / "cespy" / "cache"
        elif self.info.is_macos:
            cache_dir = Path.home() / "Library" / "Caches" / "cespy"
        else:  # Linux and other Unix-like
            cache_dir = Path(os.environ.get('XDG_CACHE_HOME', 
                                          Path.home() / ".cache")) / "cespy"
