...
class BenchmarkSuite:
    ...
    def load_baseline(self) -> None:
        ...
        try:
            ...
            _logger.info("Loaded baseline data with %d benchmarks", len(self.baseline_data))
    ...
    def benchmark_simulator_detection(self) -> Dict[str, float]:
        ...
        results = {}
        simulators = [Simulators.LTSPICE, Simulators.NGSPICE, 
                     Simulators.QSPICE, Simulators.XYCE]
        ...
        for line in component_lines:
            ...
            if match:
                ...
                if len(parts) >= 4:
                    value = parts[3]
        end_time = time.perf_counter()
        results['component_parsing_time'] = (end_time - start_time) / len(component_lines)
    ...
    def run_all_benchmarks(self) -> Dict[str, Dict[str, float]]:
        ...
        for name, method in benchmark_methods:
            try:
                ...
                _logger.info("Completed benchmark %s in %.3fs", name, end_time - start_time)
...
def create_performance_test(name: str, baseline_file: Optional[Path] = None) -> Callable:
    ...
    def test_performance():
        ...
        if baseline_file and baseline_file.exists():
            ...
            if comparison['status'] == 'failed':
                ...
                _logger.error("Performance benchmark report:\n%s", suite.generate_report())
                
                raise AssertionError(failure_msg)
        
        # Always log the current performance for reference
        _logger.info("Performance benchmark report:\n%s", suite.generate_report(include_comparison=False))
