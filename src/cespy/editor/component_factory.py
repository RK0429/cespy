...
class BaseComponent:
    ...
    def validate(self) -> List[str]:
        ...
        if template:
            ...
            if template.validation_pattern and "value" in self.attributes:
                ...
                if not re.match(
                    template.validation_pattern, str(self.attributes["value"])
                ):
    ...
    def connect_pin(self, pin: str, net: str) -> None:
        ...
        if pin not in pins:
            raise ValueError(
                f"Invalid pin '{pin}' for component type {self.component_type.value}"
            )
...
class ComponentFactory:
    ...
    def create_component(
        ...
        **attributes: Any,
        ...
        if not template:
            raise InvalidComponentError(
                str(component_type), f"No template for component type: {component_type}"
            )
