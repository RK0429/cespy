...
class OptimizationResult:
    ...
    def reduction_percentage(self) -> float:
        ...
        if self.original_line_count == 0:
            return 0.0
        return ((self.original_line_count - self.optimized_line_count) / 
                self.original_line_count * 100.0)
...
class NetlistOptimizer:
    ...
    def __init__(self, config: Optional[OptimizationConfig] = None):
        ...
        _logger.info("NetlistOptimizer initialized with level: %s", self.config.level.value)
    ...
    def optimize_netlist(self, netlist_content: str) -> Tuple[str, OptimizationResult]:
        ...
        _logger.info("Optimization complete: %d%% reduction", result.reduction_percentage)
    ...
    def _remove_dangling_components(self, lines: List[str]) -> Tuple[List[str], int]:
        ...
        dangling_nodes = {node for node, count in node_connections.items() 
                ...
                if len(parts) >= 3:
                    ...
                    for i in range(1, min(4, len(parts))):
                        if parts[i] in dangling_nodes:
                            should_remove = True
                            removed_count += 1
                            _logger.debug("Removing dangling component: %s", line.strip())
                            break
        ...
        for line in lines:
            if line.upper().strip().startswith('.MODEL'):
                ...
                if len(parts) >= 3:
                    ...
                    if model_type in essential_params and self.config.level == OptimizationLevel.AGGRESSIVE:
                        # Parse parameters
                        ...
                        filtered_params = [(p, v) for p, v in param_pairs 
                                         if p.upper() in essential]
                        ...
                        if len(filtered_params) < len(param_pairs):
                            ...
                            if filtered_params:
                                new_line += " " + " ".join([f"{p}={v}" for p, v in filtered_params])
    ...
    def analyze_optimization_impact(self, original: str, optimized: str) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'size_reduction_pct': (1 - len(optimized) / len(original)) * 100 if len(original) > 0 else 0,
        ...
        if analysis['size_reduction_pct'] > 50:
            analysis['warnings'].append("Large size reduction - verify simulation accuracy")
        
        if 'X' in analysis['component_types_affected']:
            analysis['warnings'].append("Subcircuits were modified - check hierarchical behavior")
