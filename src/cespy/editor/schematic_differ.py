...
class SchematicDiff:
    ...
    def get_summary(self) -> Dict[str, int]:
        ...
        summary = {
            "components_added": sum(1 for c in self.component_changes if c.change_type == ChangeType.ADDED),
            "components_removed": sum(1 for c in self.component_changes if c.change_type == ChangeType.REMOVED),
            "components_modified": sum(1 for c in self.component_changes if c.change_type == ChangeType.MODIFIED),
            "components_moved": sum(1 for c in self.component_changes if c.change_type == ChangeType.MOVED),
            "wires_added": sum(1 for w in self.wire_changes if w.change_type == ChangeType.ADDED),
            "wires_removed": sum(1 for w in self.wire_changes if w.change_type == ChangeType.REMOVED),
    ...
    def generate_report(self, include_details: bool = True) -> str:
        ...
        lines.append(f"  Components: +{summary['components_added']} "
...
class SchematicDiffer:
    ...
    def compare_wires(
        self,
        old_wires: List[Dict[str, Any]],
        new_wires: List[Dict[str, Any]]
    ) -> List[WireChange]:
        """Compare wire lists.
    ...
    def compare_directives(
        self,
        old_directives: List[Dict[str, str]],
        ...
        for dir_type in all_types:
            ...
            for value in new_values - old_values:
                changes.append(DirectiveChange(
                    change_type=ChangeType.ADDED,
    ...
    def _positions_differ(self, pos1: Tuple[float, float], 
