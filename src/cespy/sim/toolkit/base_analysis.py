...
class BaseAnalysis(SimAnalysis):
    ...
    def run_analysis(self) -> List[AnalysisResult]:
        ...
        self.progress_reporter.report(
            ...
            f"Complete: {successful}/{total_runs} successful"
        )
        
        return results
    
    def _run_sequential(self, all_parameters: List[Dict[str, Any]]) -> List[AnalysisResult]:
    ...
    def _run_parallel(self, all_parameters: List[Dict[str, Any]]) -> List[AnalysisResult]:
...
class StatisticalAnalysis(BaseAnalysis):
    ...
    def get_correlation_matrix(
        ...
        for name, values in data_dict.items():
            values_array = np.array(values)
            if np.sum(~np.isnan(values_array)) > len(values) * 0.5:  # At least 50% valid
...
class ParametricAnalysis(BaseAnalysis):
    ...
    def get_response_surface(
        ...
        for result in self.results:
            if result.success:
                if (param1_name in result.parameters and 
                    param2_name in result.parameters and
