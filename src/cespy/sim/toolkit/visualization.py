...
class AnalysisVisualizer:
    ...
    def plot_histogram(
        ...
        **kwargs
        ...
        # Plot histogram
        ax.hist(bin_edges[:-1], bins=bin_edges, weights=counts, 
                density=density, alpha=0.7, **kwargs)
    ...
    def plot_scatter_matrix(
        ...
        **kwargs
        ...
        # Create figure
        fig, axes = plt.subplots(n_measurements, n_measurements, 
        ...
        if show_correlation:
            corr_matrix, valid_names = analysis.get_correlation_matrix(measurement_names)
        ...
        for i, name_y in enumerate(measurement_names):
            for j, name_x in enumerate(measurement_names):
                ...
                if i == j:
                    ...
                    if len(valid_data) > 0:
                        ax.hist(valid_data, bins='auto', alpha=0.7, 
                    ...
                    if len(data_x_valid) > 0:
                        ax.scatter(data_x_valid, data_y_valid, alpha=0.6, 
                                  s=20, **kwargs)
    ...
    def plot_convergence(
        ...
        **kwargs
        ...
        ax1.axhline(np.mean(values_array), color='red', linestyle='--', 
        ...
        ax2.axhline(np.std(values_array), color='red', linestyle='--', 
        ...
        **kwargs
        ...
        for result in analysis.results:
            if (result.success and 
                parameter_name in result.parameters and
                measurement_name in result.measurements):
                
                param_val = result.parameters[parameter_name]
                meas_val = result.measurements[measurement_name]
                
                if isinstance(param_val, (int, float)) and isinstance(meas_val, (int, float)):
                    param_values.append(param_val)
                    meas_values.append(meas_val)
        
        if not param_values:
            raise ValueError(f"No data found for parameter '{parameter_name}' "
        ...
        if len(param_values) > 2:
            ...
            ax.plot(x_trend, y_trend, 'r--', alpha=0.8, 
                   label=f'Slope: {coeffs[0]:.3e}')
        
        # Calculate sensitivity
        if hasattr(analysis, 'get_parameter_sensitivity'):
            sensitivity = analysis.get_parameter_sensitivity(parameter_name, measurement_name)
    ...
    def create_analysis_report(
        ...
        if len(measurement_names) > 1:
            try:
                fig = self.plot_scatter_matrix(analysis, measurement_names, show_correlation=True)
        ...
        for measurement in measurement_names:
            if len(analysis.results) >= 100:
                try:
                    ...
                    file_path = output_path / f"{report_name}_{measurement}_convergence.png"
                    fig.savefig(file_path, dpi=300, bbox_inches='tight')
                    plt.close(fig)
                    report_files.append(file_path)
                except Exception as e:
                    _logger.warning("Failed to create convergence plot for %s: %s", measurement, e)
...
""")
            for file_path in report_files:
                if file_path.suffix == '.png':
                    f.write(f'        <li><a href="{file_path.name}">📊 {file_path.name}</a></li>\n')
                else:
                    f.write(f'        <li><a href="{file_path.name}">📄 {file_path.name}</a></li>\n')
            
            f.write("    </ul>\n</body>\n</html>")
        
        _logger.info("Analysis report created with %d files in %s", len(report_files), output_path)
        return index_file
...
def create_simple_histogram(values: List[float], title: str = "Histogram") -> Optional[Figure]:
