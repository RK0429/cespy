...
class SimulatorFactory:
    ...
    def create(
        ...
        exe_path, uses_wine = locator.find_simulator(str(custom_path) if custom_path else None)
        ...
        if validate:
            is_valid, version_or_error = locator.validate_executable(exe_path, uses_wine)
            if not is_valid:
                raise InvalidSimulatorError(
            ...
            try:
                ...
                if exe_path:
                    ...
                    info = SimulatorInfo(
                        ...
                        status=SimulatorStatus.AVAILABLE if is_valid else SimulatorStatus.INVALID_VERSION,
                        library_paths=[Path(p) for p in simulator.get_default_library_paths()],
