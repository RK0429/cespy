...
from .simulator_interface import ISimulator, SimulatorInfo, SimulatorStatus, SimulationCommand
...
class SimulatorAdapter(ISimulator):
    ...
    def validate_installation(self) -> SimulatorInfo:
        ...
        if self.simulator_class.spice_exe:
            ...
            raise SimulatorNotInstalledError(f"No executable path set for {self.simulator_name}")
    ...
    def validate_options(self, options: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        if "timeout" in options:
            if not isinstance(options["timeout"], (int, float)) or options["timeout"] <= 0:
        ...
        if "threads" in options:
            ...
            if not isinstance(options["threads"], int) or options["threads"] < 1:
                ...
                errors.append(f"threads cannot exceed {max_threads} for {self.simulator_name}")
