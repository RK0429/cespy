...
simulation servers and executing SPICE simulations in a distributed
environment. It handles job submission, monitoring, and result retrieval
...
class JobInformation:
    ...
    run_number: int  # The run id returned by the Server
...
class SimClient:
    ...
    simulation Server. Upon instance, it will establish a connection with
    Simulation Server. This connection is kept alive during the whole
    live of this object.

    The run() method will transfer the netlist for the server, execute a
    simulation and transfer the simulation results back to the client.

    Data is returned from the server inside a zipfie which is copied into
    the directory defined when the job was created, /i.e./ run() method
    called.

    Two lists are kept by this class:

    * A list of started jobs (started_jobs) and,
    * a list with finished jobs on the server, but, which haven't been
      yet transferred to the client (stored_jobs).

    This distinction is important because the data is erased on the
    server side when the data is transferred.

    This class implements an iterator that is to be used for retrieving
    the job. See the example below. The iterator polls the server with a
    time interval defined by the attribute
    ``minimum_time_between_server_calls``. This attribute is set to 0.2
    ...
    .. code-block:: python
        ...
        for runid in server:
            ...
            with zipfile.ZipFile(zip_filename, 'r') as zipf:
                # Debug printing the contents of the zip file
                print(zipf.namelist())
                # Normally the raw file comes first
                zipf.extract(zipf.namelist()[0])

    NOTE: More elaborate algorithms such as managing multiple servers
    will be done on another class.
    """
    ...
    def add_sources(self, sources: Iterable[str | pathlib.Path]) -> bool:
        ...
        try:
            ...
            _logger.error(
                "Client: Failed to add sources to session %s: %s",
                self.session_id,
                e,
            )
            return False
...
def main() -> None:  # pylint: disable=too-many-nested-blocks
    ...
    parser.add_argument(
        "host",
        help="Server hostname or IP address (e.g., localhost, 192.168.1.100)",
    )
    parser.add_argument(
        "-p",
        "--port",
    ...
    parser.add_argument(
        "-r",
        "--run",
        help="Path to circuit file to simulate",
        metavar="CIRCUIT",
    )
    parser.add_argument(
        "-d",
