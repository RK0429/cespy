...
from cespy.client_server import SimServer, SimClient, ServerSimRunner
    ...
    try:
        ...
        if successful_jobs:
            ...
            for job in successful_jobs[:3]:  # Show first 3
                ...
                print(f"  Job {job['job_id']}: R={job_params['R_val']}, "
...
"""
    ...
    try:
        ...
        for i, circuit_path in enumerate(circuits):
            runner = SimRunner()
            runner.simulator = LTspice()
            runner.set_circuit(str(circuit_path))
        ...
        with ProcessPoolExecutor(max_workers=2) as executor:
            circuit_info = [(i, path) for i, path in enumerate(circuits[:4])]  # Limit for demo
...
"""
        ...
        optimized_runner = SimRunner()
        optimized_runner.simulator = LTspice()
        optimized_runner.set_circuit(str(circuit_path))
        ...
        optimized_runner.set_circuit(str(optimized_path))
        
        # Measure optimized performance
        optimized_times = []
        for i in range(5):
            start_time = time.time()
            result = optimized_runner.run()
            elapsed = time.time() - start_time
        ...
        start_time = time.time()
        for i in range(3):
            persistent_runner = SimRunner()
            persistent_runner.simulator = LTspice()
            persistent_runner.set_circuit(str(circuit_path))
            persistent_runner.run()
        persistent_time = time.time() - start_time
        
        # Temporary file test
        start_time = time.time()
        for i in range(3):
            with tempfile.NamedTemporaryFile(mode='w', suffix='.net', delete=False) as tmp_file:
                tmp_file.write(complex_circuit)
                tmp_path = tmp_file.name
            
            temp_runner = SimRunner()
            temp_runner.simulator = LTspice()
            temp_runner.set_circuit(tmp_path)
            temp_runner.run()
