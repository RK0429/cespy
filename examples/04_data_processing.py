...
from cespy.raw import (
    RawRead, RawWrite, RawReadLazy, RawFileStreamer,
    RawDataCache, OptimizedBinaryParser
)
...
def create_sample_raw_data():
    ...
    vout_time = 0.8 * np.sin(2 * np.pi * 1000 * time_points - 0.1)  # Delayed, attenuated
    
    # Frequency domain signals
    gain_freq = 20 * np.log10(1 / np.sqrt(1 + (frequency_points / 1000)**2))  # Low-pass response
...
def example_lazy_loading():
    ...
    try:
        ...
        large_signal = np.sin(2 * np.pi * 100 * large_time) + 0.1 * np.random.randn(len(large_time))
        ...
        for i in range(min(5, total_chunks)):  # Process first 5 chunks
            ...
            print(f"  Chunk {i+1}: points {start_idx}-{end_idx}, mean = {chunk_mean:.3f}")
...
def example_data_streaming():
    ...
    try:
        ...
        for i in range(3):
            ...
            signal_data = np.sin(2 * np.pi * (1000 + i * 100) * time_data)  # Different frequencies
        ...
        for result in processed_results:
            print(f"  File {result['file']}: {result['dominant_frequency']:.1f} Hz, "
...
def example_data_caching():
    ...
    try:
        ...
        for i in range(3):
            ...
            signal_data = np.sin(2 * np.pi * 1000 * time_data) * np.exp(-time_data * 100)
...
def example_histogram_analysis():
    ...
    try:
        ...
        print(f"Fitted normal parameters: μ={fitted_params['mu']:.3f}, σ={fitted_params['sigma']:.3f}")
        ...
        print(f"  Distributions are {'similar' if comparison['ks_test_p'] > 0.05 else 'different'}")
...
def example_visualization():
    ...
    try:
        ...
        plt.subplot(2, 2, 1)
        plt.plot(sample_data['transient']['time'] * 1e3, 
                sample_data['transient']['vin'], 'b-', label='Input')
        plt.plot(sample_data['transient']['time'] * 1e3, 
                sample_data['transient']['vout'], 'r-', label='Output')
        plt.xlabel('Time (ms)')
        ...
        # Transient analysis
        axes[0, 0].plot(sample_data['transient']['time'] * 1e6, 
        ...
        axes[0, 1].plot(sample_data['transient']['time'] * 1e6, 
        ...
        axes[0, 2].plot(sample_data['transient']['vin'], 
                       sample_data['transient']['vout'])
        ...
        # AC analysis
        axes[1, 0].loglog(sample_data['ac']['frequency'], 
                         10**(sample_data['ac']['gain']/20))
        axes[1, 0].set_title('Bode Plot - Magnitude')
        axes[1, 0].set_xlabel('Frequency (Hz)')
        axes[1, 0].set_ylabel('Magnitude')
        axes[1, 0].grid(True)
        
        axes[1, 1].semilogx(sample_data['ac']['frequency'], 
                           sample_data['ac']['phase'])
